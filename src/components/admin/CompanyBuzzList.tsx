...
import { MessageSquare, User, Calendar, Search, X, Filter, ChevronDown, Bookmark, Share2, ThumbsUp, Clock } from 'lucide-react';
...
const CompanyBuzzList: React.FC = () => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-12">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-blue-100 mt-1 text-sm sm:text-base">Stay updated with company news and announcements</p>
              </div>
            </div>
          ...
          <div className="mt-4 bg-white/10 backdrop-blur-md rounded-lg px-4 py-2 text-sm text-blue-100">
            <div className="flex items-center">
              ...
              <p>Some filter options are coming soon! Currently filtering is fully enabled for displayed categories.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-12"> {/* Increased margin-top */}
          ...
          viewMode === 'grid' ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredPosts.map((post, index) => {
                ...
                const mockDate = new Date(Date.now() - Math.floor(Math.random() * 10) * 24 * 60 * 60 * 1000).toISOString();
                    ...
                    <div className="p-4 sm:p-6 flex-1">
                      ...
                      <div className="flex flex-col mb-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className={`text-xs font-medium px-2.5 py-1 rounded-full inline-block max-w-[150px] truncate ${getCategoryBadgeColor(post.category || mockCategory)}`}>
                      ...
                      <h3 className="font-bold text-xl text-gray-800 mb-2 line-clamp-2 break-words">{post.title}</h3>
                      
                      {/* Text content with proper overflow handling */}
                      <p className="text-gray-600 mb-4 line-clamp-3 break-words">{post.textContent}</p>
                      ...
                      <div className="flex items-center mt-4">
                        ...
                        <div className="ml-3 min-w-0 max-w-[calc(100%-50px)]">
                          <p className="text-sm font-medium text-gray-900 truncate">{post.CompanyUser.name}</p>
                          <p className="text-xs text-gray-500 truncate">{post.CompanyUser.designation}</p>
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                              <h3 className="font-bold text-lg text-gray-900 break-words">{post.title}</h3>
                              <span className={`text-xs font-medium px-2.5 py-1 rounded-full self-start ${getCategoryBadgeColor(post.category || mockCategory)}`}>
                                {post.category || mockCategory}
                              </span>
                            </div>
                            ...
                            <p className="text-gray-600 mt-2 mb-3 break-words">{post.textContent}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-6">
          <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 shadow-sm">
            <p className="text-gray-600 text-sm">
              Showing <span className="font-semibold text-blue-600">{filteredPosts.length}</span> of <span className="font-semibold">{posts.length}</span> total posts
              {searchTerm && <span> matching "<span className="font-medium">{searchTerm}</span>"</span>}
              {activeFilter !== 'all' && <span> in <span className="font-medium">{activeFilter}</span></span>}
            </p>
          </div>
        </div>
      )}
...
function getCategoryColor(category: string): string {
  switch (category) {
    ...
    case 'Insights': return 'bg-amber-500';
...
function getCategoryBadgeColor(category: string): string {
  switch (category) {
    ...
    case 'Insights': return 'bg-amber-100 text-amber-800';
